...
document.querySelector("#answer-form").addEventListener("submit", (event) => {
  ...
  const row = { ...createTaskSession("conjunction-fallacy", "1.1.0"), item: "linda_problem", response: answer, correctness: correct ? 1 : 0, option_order: order.map((item) => item.value).join("|"), rt: Number((performance.now() - startedAt).toFixed(3)) };
