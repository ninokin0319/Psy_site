...
document.querySelector("#config-form").addEventListener("submit", (event) => {
  ...
  session = createTaskSession("anchoring", "1.2.0");
  anchor = getAnchor(new FormData(event.currentTarget).get("anchorCondition"), seededRandom(session.session_id));
...
document.querySelector("#answer-form").addEventListener("submit", (event) => {
  ...
  if (!familiarity || !Number.isFinite(confidence)) {
    document.querySelector(".question-error").textContent = "事前知識と確信度も回答してください。";
    return;
  }
  const row = { ...session, anchor_condition: anchor.condition, anchor_value: anchor.value, comparison_response: comparison, comparison_correctness: comparison === (actual < anchor.value ? "before" : "after") ? 1 : 0, estimate_year: estimate, prior_familiarity: familiarity, confidence_percent: confidence, actual_year: actual, signed_error: estimate - actual, absolute_error: Math.abs(estimate - actual), rt: Number((performance.now() - startedAt).toFixed(3)) };
